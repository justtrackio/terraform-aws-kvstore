locals {
  ...
  ddb_autoscaling_read = var.ddb_billing_mode == "PROVISIONED" ? length(var.ddb_autoscaling_read) == 0 ? {
    max_capacity = 100
  } : var.ddb_autoscaling_read : {}
  ddb_autoscaling_write = var.ddb_billing_mode == "PROVISIONED" ? length(var.ddb_autoscaling_write) == 0 ? {
    max_capacity = 100
