...
| <a name="input_label_orders"></a> [label\_orders](#input\_label\_orders) | Overrides the `labels_order` for the different labels to modify ID elements appear in the `id` | <pre>object({<br>    kvstore = optional(list(string), ["environment", "stage", "tenant", "name"])<br>  })</pre> | `{}` | no |
